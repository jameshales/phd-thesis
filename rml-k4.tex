...
Therefore by induction on the structure of $\phi$ we have for every $\kPModel{\kStateS} \in \classKF$: $\kPModel{\kStateS} \entails \phi^{(\agentsB, \atomP)}$ if and only if $\kPModel[(\agentsB, \atomP)]{\kStateS} \entails \phi$.
\end{proof}
...
However as one designated state has an appropriate infinite path and the other doesnt', they disagree on the intepretation of the given \langMu{} formula.
