...
That \logicRmlKF{} is at least as expressive as \logicKF{} is obvious, as \logicRmlKF{} generalises hte syntax and semantics of \logicKF{}.
To show that \logicRmlKF{} is strictly more expressive than \logicKF{} we demonstrate a \classKF{} Kripke model with two states that can be distinguished by the validity of a \langRml{} formula under the semantics of \logicRmlKF{}, but that cannot be distinguished by any \langMl{} formula under the semantics of \logicKF{}. 
...
We used a similar approach in Section~\ref{rml-k4-ml} to show that \logicRmlKF{} is strictly more expressive than \logicKF{}.
In that result we relied on the fact that $n$-bisimililarity for all $n \in \naturals$ implies modal equivalence.
Here we introduce a similar notion of $n$-mutual refinements.
...
In our expressivity result we will rely on the fact that two states being $n$-mutual refinements for all $n \in \naturals$ implies refinement modal equivalence.
...
and so $\kPModel{\omega}$ and $\kPModel{\omega'}$ are distinguishable by the modal $\mu$-calculus.
\end{proof}
