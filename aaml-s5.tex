...
\begin{corollary}
The model-checking and satisfiability problems for the logic \logicRamlS{} are decidable.
\end{corollary}
...
\begin{eqnarray*}
    ...
    \aPrecondition &=& \{(\aStateTest, \someactsBs \phi)\} \cup \{(\aPStateT[\agentA, \gamma], \aPrecondition[\agentA, \gamma](\aStateT[\agentA, \gamma])) \mid \agentA \in \agents, \gamma \in \Gamma_\agentA, \aStateT[\agentA, \gamma] \in \aStatesT[\agentA, \gamma]\} \cup \\&&\quad \bigcup_{\agentA \in \agents, \gamma \in \Gamma_\agentA} \aPrecondition[\agentA, \gamma]
\end{eqnarray*}
...
This is similar to the construction used to show the soundness of the axiom {\bf RS5} in \logicRmlS{}, but it deals with all agents in $\agentsC$ at once, rather than a single agent at a time.
...
\begin{corollary}
The model-checking and satisfiability problems for the logic \logicAamlS{} are decidable.
\end{corollary}
