...
In the following sections we provide an axiomatisation for \logicRmlK{} and show that is sound and complete with respect to the semantics of \logicRmlK{}.
In Section~\ref{rml-k-axiomatisation} we provide the axiomatisation for \logicRmlK{} and show some preliminary results about the axiomatisation.
...
Instead we give the axioms {\bf RK}, {\bf RComm}, and {\bf RDist} which consider conjunctions of modalities rather than single modalities, and use the cover operators as a convenient notation for a conjunction of an arbitrary number of modalities.
In Section~\ref{rml-k-soundness} we find that the semantics of the cover operator are convenient for showing the soundness of these axioms.
Finally in Section~\ref{rml-k-completeness} we see that the cover operator allows a convenient disjunctive normal form for modal formulas, which we use in our provably correct translation from \langRml{} to \langMl{}.
...
Each of these axioms share the general form of equivalences, where the left hand side describes the existence of a single refinement that satisfies a given formula, whilst the right hand side describes the existence of multiple refinements that satisfy subformulas of the given formula.
...
Conversely, for the right-to-left direction of the equivalence, we show that if we have all of the refinements described on the right hand side of the equivalence, then these refinements can be combined into a single refinement that satisfies the left hand side of the equivalence.
...
Our syntax differs from Janin and Walukiewicz~\cite{janin:1995} primarily to be consistent with the established literature in \logicRml{}~\cite{vanditmarsch:2009,vanditmarsch:2010}.
...
$$
where $\pi \in \langMl$, $\agentsB \subseteq \agents$, and for every $\agentB \in \agentsB$, $\Gamma_\agentB \subseteq \langMl$ is a finite set of formulas in disjunctive normal form.
\end{definition}
...
\begin{align}
    ...
    \proves & \displaystyle \somerefsBs (\pi \land \bigwedge_{\agentC \in \agentsC} \coverC \Gamma_\agentC) \iff \nonumber\\
            ...
            (
            \pi \land
            \bigwedge_{\agentC \in \agentsC \cap \agentsB} \coversC (\{\somerefsBs \gamma \mid \gamma \in \Gamma_\agentC\} \cup \{\top\}) \land
            \bigwedge_{\agentC \in \agentsC \setminus \agentsB} \coversC \{\somerefsBs \gamma \mid \gamma \in \Gamma_\agentC\} 
            ) \label{rml-k-cover}
\end{align}
where $\pi \in \langPl$, $\phi, \psi \in \langRml$, $\agentsB \subseteq \agents$, and for every $\agentB \in \agentsB$, $\Gamma_\agentB \subseteq \langRml$ is a finite set of formulas.
\end{lemma}
...
\begin{description}
    ...
    \begin{align*}
        \proves & \displaystyle \somerefsBs (\pi \land \bigwedge_{\agentC \in \agentsC} \coverC \Gamma_\agentC) \iff \\&\quad
            (
            \pi \land
            \bigwedge_{\agentC \in \agentsC \cap \agentsB} \coversA (\{\somerefsBs \gamma \mid \gamma \in \Gamma_\agentC\} \cup \{\top\}) \land
            \bigwedge_{\agentC \in \agentsC \setminus \agentsB} \coversC \{\somerefsBs \gamma \mid \gamma \in \Gamma_\agentC\} 
            )
    \end{align*}
...
We can now clearly recognise that the equivalences (\ref{rml-k-or}) and (\ref{rml-k-covers}) are reduction axioms that can be used to push refinement quantifiers past propositional connectives and modalities in formulas in disjunctive normal form.
...
Secondly we show that \axiomRmlK{} satisfies substitution of equivalents.
...
Given the provably correct translation we have that \logicRmlK{} is sound and complete.
