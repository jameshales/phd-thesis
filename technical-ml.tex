...
\begin{figure}
    ...
    \begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=7em,thick]
        ...
        \node[label=left:{$\kStateU$}] (u) [below of=s] {$\{\atomQ\}$};
        \node[label=right:{$\kStateV$}] (v) [right of=u] {$\{\}$};

      \path[every node/.style={font=\sffamily\small},->]
        (s) edge [loop above] node {$\agentA, \agentB$} (s)
        (t) edge [loop above] node {$\agentA, \agentB$} (t)
        (u) edge [loop below] node {$\agentA, \agentB$} (u)
...
As $\kStateS, \kStateT \in \kValuation(\atomP)$ then $\kPModel{\kStateS} \entails \atomP$ and $\kPModel{\kStateT} \entails \atomP$.
As $\kSuccessorsA{\kStateS} = \{\kStateS, \kStateT\}$ then $\kPModel{\kStateS} \entails \necessaryA \atomP$.
However as $\kStateU \notin \kValuation(\atomP)$ then $\kPModel{\kStateU} \nentails \atomP$,
and as $\kStateU \in \kSuccessorsB{\kStateS}$ then $\kPModel{\kStateS} \nentails \necessaryB \atomP$.
So we have $\kPModel{\kStateS} \entails \lnot \necessaryB \atomP$ and likewise $\kPModel{\kStateT} \entails \lnot \necessaryB \atomP$ therefore $\kPModel{\kStateS} \entails \necessaryA \lnot \necessaryB \atomP$.
\end{example}
...
If there exists a bisimulation $\bisimulation$ that is a total relation between $\kStates$ and $\kStatesP$ then we say that $\kModel$ and $\kModelP$ are {\em bisimilar} and we denote this by $\kModel \bisimilar \kModelP$.
