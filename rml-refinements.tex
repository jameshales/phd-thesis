...
In this section we introduce refinements, which are relations from a Kripke model to another Kripke model that in epistemic settings correspond to a notion that the latter is the result of an epistemic update of the former.
In Section~\ref{ml} we introduced bisimulations, which are relations between Kripke models that partially correspond to the notion of modal equivalence.
...
A refinement is a generalisation of a bisimulation, that allows the condition {\bf forth} to be relaxed for a given set of agents.
...
However as refinements over different sets of agents satisfy the condition {\bf forth} for different sets of agents, then composing refinements over different sets of agents results in a refinement over the union of the two sets of agents.
...
\begin{proof}
We will show that the relation $\refinement'' = \refinement \circ \refinement' \subseteq \kStates \times \kStatesPP$ is a $(\agentsB \cup \agentsC)$-refinement and therefore $\kPModel{\kStateS} \simulatesBs \kPModelPP{\kStateSPP}$.
Let $\atomP \in \atoms$, $\agentA \in \agents$, $\agentC \in \agents \setminus (\agentsB \cap \agentsC)$ and $(\kStateS, \kStateSPP) \in \refinement''$.
...
Let $(\kStateS, \kStateSPP) \in \refinement''$ where there exists $\kStateSP \in \kStatesP$ such that $(\kStateS, \kStateSP) \in \refinement$ and $(\kStateSP, \kStateSPP) \in \refinement'$.
...
As $\agentC \in \agents \setminus (\agentsB \cap \agentsC)$ then $\agentC \in \agents \setminus \agentsB$ and $\agentC \in \agents \setminus \agentsC$, so {\bf forth-$\agentC$} holds for $\refinement$ and $\refinement'$.
...
Therefore $\refinement''$ is a $(\agentsB \cup \agentsC)$-refinement.
\end{proof}
...
By Proposition~\ref{refinement-composition} the relation the relation $\refinement'' = \refinement \circ \refinement' \subseteq \kStates \times \kStatesPP$ is a $\agentsB$-refinement and therefore $\kPModel{\kStateS} \simulatesBs \kPModelPP{\kStateSPP}$.
\end{proof}
...
The refinements of a Kripke model may be characterised intuitively as being formed by taking a bisimilar Kripke model (satisfying {\bf atoms}, {\bf forth}, and {\bf back}) and then removing edges for some agents (relaxing {\bf forth} for some agents).
...
Then $\refinement$ is an {\em expanded $\agentsB$-refinement} if and only if for every $\kStateSP \in \kStatesP$ there is a unique $\kStateS \in \kStates$ such that $(\kStateS, \kStateSP) \in \refinement$.
\end{definition}
...
We show that $\refinement$ is a $\agentsB$-refinement from $\kModel$ to $\kModelP$.
...
We show that $\refinement'$ is a $\agentsC$-refinement from $\kModel$ to $\kModelP$.
...
Compared to bisimulations, $\agentsB$-refinements relax the {\bf forth} condition for the agents in $\agentsB$, and this is why the truth of $\possibleB$ operators for $\agentB \in \agentsB$ are not preserved by refinements in general.
...
If we consider the preservation of positive formulas to be a minimal requirement for our notion of epistemic updates, then taken together Proposition~\ref{refinements-preserve-positive} and Proposition~\ref{refinements-hennessy-milner} form a strong case in favour of refinements as corresponding to a very general notion of epistemic updates, just as the analogous results for bisimulations form a strong case in favour of bisimulations as corresponding to the notion of modal equivalence.

Of course, refinements are not the {\em most general} notion of epistemic updates, as for example, models of belief revision permit agents to forget or revise previous information~\cite{alchourron:1985}, so such updates do not preserve positive formulas.
...
Whereas van Ditmarsch and French~\cite{vanditmarsch:2009} showed this result with the use of the common knowledge operator, we restate this result without the common knowledge operator.
