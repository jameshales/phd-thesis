...
We note that the above definition is not recursive; $\agentsB$-bisimilarity is defined in terms of regular bisimilarity.
Intuitively two Kripke models are $\agentsB$-bisimilar if their $\agentsB$-successors are bisimilar.
...
We note that the translation used in the proof of Lemma~\ref{dnf-equivalent} does not introduce modalities from agents at the top level of the formula, not already appearing at the top level of the formula, so translating a $\agentsB$-restricted modal formula into disjunctive normal form will result in a $\agentsB$-restricted modal formula.
...
Here we note that for every $\agentC \in \agentsC$, $\gamma' \in \Gamma'_\agentC$ may contain $\coversC$ modalities at the top level, and so we use some equivalences to correct this.
...
\end{align}
where $\phi, \psi \in \langRml$, $\pi \in \langPl$, $\agentA \in \agents$, $\agentsB, \agentsC \subseteq \agents$, and for every $\agentA \in \agents$: $\Gamma_\agentA$ is a finite, non-empty set of $(\agents \setminus \{\agentA\})$-restricted modal formulas.
\end{lemma}
...
Every refinement modal formula is provably equivalent to a modal formula using the axiomatisation \axiomRmlKD{}.
\end{lemma}
